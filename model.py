...
from param_shared_transformer_encoder import ParameterSharedTransformerEncoder
#from VariousEncoders.transformers import TransformerEncoder
#from fairseq.simple_lstm import SimpleLSTMEncoder
...
class Model(nn.Module):
    ...
    def __init__(self,
                 encoder: ParameterSharedTransformerEncoder,#TransformerEncoder,
                 decoder: Decoder,
    ...
    def forward(self,
        ...
        skel_out, dec_hidden, _, _ = self.decode(encoder_output=encoder_output,
                                                 src_mask=torch.cuda.BoolTensor(src_mask), trg_input=trg_input,
    ...
    def encode(self, src: Tensor, src_length: Tensor, src_mask: BoolTensor):
        """
        ...
        ## Base + the three in the code
        #encode_output, encode_hidden = self.encoder(self.src_embed(src), src_length, src_mask)
        
        encode_output = self.encoder(self.src_embed(src), src_length, src_mask)
        # encode_output = self.encoder(self.src_embed(src), src_length)
        print("encoder output: ",encode_output.shape)
        # encode_output = self.encoder(self.src_embed(src), src_length, src_key_padding_mask=src_mask)
    ...
    def decode(self, encoder_output: Tensor,
        ...
        decoder_output = self.decoder(trg_embed=trg_embed, encoder_output=encoder_output,
                               src_mask=torch.cuda.BoolTensor(src_mask),trg_mask=trg_mask) #.to('cuda')
    ...
    def get_loss_for_batch(self, batch: Batch, loss_function: nn.Module) \
        ...
        skel_out, _ = self.forward(
            ...
            src_mask=torch.cuda.BoolTensor(batch.src_mask), src_lengths=batch.src_lengths,
            trg_mask=batch.trg_mask) #.to('cuda')
    ...
    def run_batch(self, batch: Batch, max_output_length: int,):
        ...
        encoder_output = self.encode(
            ...
            torch.cuda.BoolTensor(batch.src_mask))
        ...
        stacked_output, stacked_attention_scores = greedy(
                ...
                src_mask=torch.cuda.BoolTensor(batch.src_mask),
...
def build_model(cfg: dict = None,
    ...
    # # Replace the instantiation of the encoder
    # print("hidden_size: ",cfg["encoder"]["hidden_size"])

    # print("num_heads: ", cfg["encoder"]["num_heads"])
    # print("ff_size: ", cfg["encoder"]["ff_size"])

    # encoder = TransformerEncoder(emb_dim=cfg["encoder"]["embeddings"]["embedding_dim"],
    #                              num_heads=cfg["encoder"]["num_heads"],
    #                              num_layers=cfg["encoder"]["num_layers"],
    #                              positional_encoding="rel")
    
    # encoder = SimpleLSTMEncoder(dictionary={1:'a'}, embed_dim=cfg["encoder"]["embeddings"]["embedding_dim"], 
    #                             hidden_dim=cfg["encoder"]["hidden_size"],
    #                             )

    encoder = ParameterSharedTransformerEncoder(
        d_model=cfg["encoder"]["hidden_size"],
        nhead=cfg["encoder"]["num_heads"],
        dim_feedforward=cfg["encoder"]["ff_size"],
        dropout=cfg["encoder"].get("dropout", 0.1),
